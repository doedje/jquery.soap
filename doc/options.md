...
But it is also allowed to just pass in the XML that should go in the body, the envelope and body node will be generated by **$.soap** for you:
```
var xml =
	['<requestNode>',
		'<name>Remy Blom</name>',
...
```
In this example `data` is a function **receiving** $.soap's SOAPObject that can be used to create and **return** an instance of the SOAPObject class.
```
$.soap({
